...
// CategoriesDao 商品分类表的Dao实现
type CategoriesDao struct {
	*gorm.DB
}

func (dao *CategoriesDao) Database() string {
	// TODO 补全 db 名称
	return "@database_name"
}

// NewCategoriesDao 创建CategoriesDao实例
// 参数:
//   - db: GORM数据库连接实例
//
// 返回:
...
//	})
func (dao *CategoriesDao) WithTx(tx *gorm.DB) *CategoriesDao {
	return &CategoriesDao{DB: tx}
}
...
// 使用示例:
//
//	err := dao.Transaction(ctx, func(txDao *CategoriesDao) error {
...
//	    return nil
//	})
func (dao *CategoriesDao) Transaction(ctx context.Context, fn func(*CategoriesDao) error) error {
	return dao.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		txDao := &CategoriesDao{DB: tx}
		return fn(txDao)
	})
}
...
// buildCategoriesQueryCondition 构建查询条件
// 参数:
//   - db: GORM数据库连接实例
//   - queryDto: 查询条件Dto对象
//
...
func (dao *CategoriesDao) buildCategoriesQueryCondition(db *gorm.DB, queryDto *query.CategoriesDto) (*gorm.DB, error) {
	if queryDto == nil {
		return db, nil
	}
	...
	if queryDto.ParentId != nil {
		db = db.Where("parent_id = ?", queryDto.ParentId)
	}
	if queryDto.Icon != nil && *queryDto.Icon != "" {
		db = db.Where("icon = ?", *queryDto.Icon)
	}
	...
	if queryDto.IconFuzzy != nil && *queryDto.IconFuzzy != "" {
		db = db.Where("icon LIKE ?", "%"+*queryDto.IconFuzzy+"%")
	}
	...
	// IN 查询条件
	// 校验 ParentIdList: 如果不为 nil 但长度为 0，则报错
	if queryDto != nil && queryDto.ParentIdList != nil {
		if len(queryDto.ParentIdList) == 0 {
			return nil, fmt.Errorf("ParentIdList 不能为空列表")
		}
		db = db.Where("parent_id IN ?", queryDto.ParentIdList)
	}
...
//     2. List 不为 nil 且长度 > 0: 添加 IN 查询条件
//     3. List 不为 nil 但长度 = 0: 返回错误，因为空列表的 IN 查询没有意义，应提前发现此问题
func (dao *CategoriesDao) SelectList(ctx context.Context, queryDto *query.CategoriesDto) ([]*model.Categories, error) {
	var resultList []*model.Categories
	db := dao.WithContext(ctx).Model(&model.Categories{})

	// 应用查询条件
	var err error
	db, err = dao.buildCategoriesQueryCondition(db, queryDto)
	if err != nil {
		return nil, err
	}
...
//     2. List 不为 nil 且长度 > 0: 添加 IN 查询条件
//     3. List 不为 nil 但长度 = 0: 返回错误，因为空列表的 IN 查询没有意义，应提前发现此问题
func (dao *CategoriesDao) SelectCount(ctx context.Context, queryDto *query.CategoriesDto) (int64, error) {
	var count int64
	db := dao.WithContext(ctx).Model(&model.Categories{})
...
func (dao *CategoriesDao) SelectListWithAppendConditionFunc(ctx context.Context, queryDto *query.CategoriesDto, appendConditionFunc func(ctx context.Context, db *gorm.DB) *gorm.DB) ([]*model.Categories, error) {
	var resultList []*model.Categories
	db := dao.WithContext(ctx).Model(&model.Categories{})
...
func (dao *CategoriesDao) SelectCountWithAppendConditionFunc(ctx context.Context, queryDto *query.CategoriesDto, appendConditionFunc func(ctx context.Context, db *gorm.DB) *gorm.DB) (int64, error) {
	var count int64
	db := dao.WithContext(ctx).Model(&model.Categories{})
...
func (dao *CategoriesDao) Insert(ctx context.Context, poBean *model.Categories) error {
	if poBean == nil {
...
func (dao *CategoriesDao) InsertBatch(ctx context.Context, poBeanList []*model.Categories) error {
	if len(poBeanList) == 0 {
...
func (dao *CategoriesDao) InsertOrUpdateNullable(ctx context.Context, poBean *model.Categories) error {
	if poBean == nil {
...
func (dao *CategoriesDao) InsertOrUpdateBatchNullable(ctx context.Context, poBeanList []*model.Categories) error {
	if len(poBeanList) == 0 {
...
// ==================== 普通索引 idx_parent_id 方法 ====================

// SelectByParentId 根据索引idx_parent_id查询列表
// 参数:
//   - ctx: 上下文对象
...
// 说明:
//   - 该索引不是唯一索引，可能返回多条记录
func (dao *CategoriesDao) SelectByParentId(ctx context.Context, parentId *uint64) ([]*model.Categories, error) {
	var resultList []*model.Categories
	err := dao.WithContext(ctx).Where("parent_id = ?", parentId).Find(&resultList).Error
	return resultList, err
}

// SelectByParentIdList 根据索引idx_parent_id批量查询列表
// 参数:
//   - ctx: 上下文对象
//   - parentIdList: 父分类ID（0表示顶级分类）列表
//
// 返回:
//   - [] *model.Categories: 查询结果列表
//   - error: 错误信息
func (dao *CategoriesDao) SelectByParentIdList(ctx context.Context, parentIdList []*uint64) ([]*model.Categories, error) {
	if len(parentIdList) == 0 {
		return []*model.Categories{}, nil
	}
	var resultList []*model.Categories
	err := dao.WithContext(ctx).Where("parent_id IN ?", parentIdList).Find(&resultList).Error
	return resultList, err
}

// UpdateByParentId 根据索引idx_parent_id更新（不会用零值覆盖）
// 参数:
//   - ctx: 上下文对象
//   - poBean: 包含更新数据的PO对象
//   - parentId: 父分类ID（0表示顶级分类）
...
func (dao *CategoriesDao) UpdateByParentId(ctx context.Context, poBean *model.Categories, parentId *uint64) error {
	if poBean == nil {
		return fmt.Errorf("更新对象不能为空")
	}
	return dao.WithContext(ctx).Model(&model.Categories{}).Where("parent_id = ?", parentId).Updates(poBean).Error
}

// UpdateByParentIdWithMap 根据索引idx_parent_id使用Map更新指定字段（可以用零值覆盖）
// 参数:
//   - ctx: 上下文对象
//   - parentId: 父分类ID（0表示顶级分类）
...
//   - 注意: 该索引不是唯一键，可能会更新多条记录
func (dao *CategoriesDao) UpdateByParentIdWithMap(ctx context.Context, parentId *uint64, updatedMap map[string]interface{}) error {
	if len(updatedMap) == 0 {
		return fmt.Errorf("更新字段不能为空")
	}
	return dao.WithContext(ctx).Model(&model.Categories{}).Where("parent_id = ?", parentId).Updates(updatedMap).Error
}

// UpdateByParentIdWithCondition 根据索引idx_parent_id和额外条件更新（不会用零值覆盖）
// 参数:
//   - ctx: 上下文对象
//   - poBean: 包含更新数据的PO对象
//   - parentId: 父分类ID（0表示顶级分类）
...
//   - 注意: 可能会更新多条记录
func (dao *CategoriesDao) UpdateByParentIdWithCondition(ctx context.Context, poBean *model.Categories, parentId *uint64, conditionMap map[string]interface{}) error {
	if poBean == nil {
		return fmt.Errorf("更新对象不能为空")
	}
	db := dao.WithContext(ctx).Model(&model.Categories{}).Where("parent_id = ?", parentId)
...
// UpdateByParentIdWithMapAndCondition 根据唯一索引idx_parent_id和额外条件使用Map更新指定字段（可以用零值覆盖）
// 参数:
//   - ctx: 上下文对象
//   - parentId: 父分类ID（0表示顶级分类）
//   - updatedMap: 要更新的字段Map
...
//   - 使用 map 可以显式指定要更新的字段，包括零值字段
//   - 提供最灵活的更新控制方式
func (dao *CategoriesDao) UpdateByParentIdWithMapAndCondition(ctx context.Context, parentId *uint64, updatedMap map[string]interface{}, conditionMap map[string]interface{}) error {
	if len(updatedMap) == 0 {
		return fmt.Errorf("更新字段不能为空")
	}
	db := dao.WithContext(ctx).Model(&model.Categories{}).Where("parent_id = ?", parentId)
...
// DeleteByParentId 根据索引idx_parent_id删除
// 参数:
//   - ctx: 上下文对象
//   - parentId: 父分类ID（0表示顶级分类）
//
...
func (dao *CategoriesDao) DeleteByParentId(ctx context.Context, parentId *uint64) error {
	return dao.WithContext(ctx).Where("parent_id = ?", parentId).Delete(&model.Categories{}).Error
}
...
// 说明:
//   - 该索引不是唯一索引，可能返回多条记录
func (dao *CategoriesDao) SelectBySortOrder(ctx context.Context, sortOrder *int) ([]*model.Categories, error) {
	var resultList []*model.Categories
...
//   - [] *model.Categories: 查询结果列表
//   - error: 错误信息
func (dao *CategoriesDao) SelectBySortOrderList(ctx context.Context, sortOrderList []*int) ([]*model.Categories, error) {
...
//   - 只更新非零值字段，零值字段会被忽略
//   - 注意: 该索引不是唯一键，可能会更新多条记录
func (dao *CategoriesDao) UpdateBySortOrder(ctx context.Context, poBean *model.Categories, sortOrder *int) error {
	if poBean == nil {
		return fmt.Errorf("更新对象不能为空")
	}
	return dao.WithContext(ctx).Model(&model.Categories{}).Where("sort_order = ?", sortOrder).Updates(poBean).Error
}
...
//   - 只更新 map 中指定的字段，未指定的字段保持不变
//   - 注意: 该索引不是唯一键，可能会更新多条记录
func (dao *CategoriesDao) UpdateBySortOrderWithMap(ctx context.Context, sortOrder *int, updatedMap map[string]interface{}) error {
	if len(updatedMap) == 0 {
		return fmt.Errorf("更新字段不能为空")
	}
	return dao.WithContext(ctx).Model(&model.Categories{}).Where("sort_order = ?", sortOrder).Updates(updatedMap).Error
}
...
//   - 适用场景: 需要在索引基础上增加额外的更新条件，缩小更新范围
//   - 注意: 可能会更新多条记录
func (dao *CategoriesDao) UpdateBySortOrderWithCondition(ctx context.Context, poBean *model.Categories, sortOrder *int, conditionMap map[string]interface{}) error {
	if poBean == nil {
		return fmt.Errorf("更新对象不能为空")
	}
	db := dao.WithContext(ctx).Model(&model.Categories{}).Where("sort_order = ?", sortOrder)
...
//   - 使用 map 可以显式指定要更新的字段，包括零值字段
//   - 提供最灵活的更新控制方式
func (dao *CategoriesDao) UpdateBySortOrderWithMapAndCondition(ctx context.Context, sortOrder *int, updatedMap map[string]interface{}, conditionMap map[string]interface{}) error {
	if len(updatedMap) == 0 {
		return fmt.Errorf("更新字段不能为空")
	}
	db := dao.WithContext(ctx).Model(&model.Categories{}).Where("sort_order = ?", sortOrder)
...
// 说明:
//   - 注意: 该索引不是唯一键，可能会删除多条记录
func (dao *CategoriesDao) DeleteBySortOrder(ctx context.Context, sortOrder *int) error {
	return dao.WithContext(ctx).Where("sort_order = ?", sortOrder).Delete(&model.Categories{}).Error
}
...
//	err := dao.ExecSql(ctx, &result, "SELECT * FROM categories WHERE id = ?", 1)
//	if err != nil {
//	    // 处理错误（包括记录不存在的情况）
//	    return err
//	}
//
//	// 示例2: 查询多条记录
//	var resultList [] *entity.Categories
//	err := dao.ExecSql(ctx, &resultList, "SELECT * FROM categories WHERE skill_id = ?", "skill123")
//	if err != nil {
//	    return err
//	}
//
...
//	err := dao.ExecSql(ctx, &countList, "SELECT skill_id, COUNT(*) as count FROM categories GROUP BY skill_id")
//
// 注意事项:
...
func (dao *CategoriesDao) ExecSql(ctx context.Context, recvPtr any, sql string, args ...any) error {
...
func (dao *CategoriesDao) getValidOrderByFields() map[string]bool {
...
func (dao *CategoriesDao) isValidOrderBy(orderBy string) bool {
	if orderBy == "" {
		return false
	}
