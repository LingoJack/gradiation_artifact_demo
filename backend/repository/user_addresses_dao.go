...
// UserAddressesDao 用户地址表的Dao实现
type UserAddressesDao struct {
	*gorm.DB
}

func (dao *UserAddressesDao) Database() string {
	// TODO 补全 db 名称
	return "@database_name"
}

// NewUserAddressesDao 创建UserAddressesDao实例
// 参数:
//   - db: GORM数据库连接实例
//
// 返回:
//   - *UserAddressesDao: Dao实例
func NewUserAddressesDao(db *gorm.DB) *UserAddressesDao {
	return &UserAddressesDao{DB: db}
}
...
//
// 返回:
//   - *UserAddressesDao: 使用事务的新 DAO 实例
//
// 使用示例:
//
...
//	})
func (dao *UserAddressesDao) WithTx(tx *gorm.DB) *UserAddressesDao {
	return &UserAddressesDao{DB: tx}
}
...
// 使用示例:
//
//	err := dao.Transaction(ctx, func(txDao *UserAddressesDao) error {
...
//	    return nil
//	})
func (dao *UserAddressesDao) Transaction(ctx context.Context, fn func(*UserAddressesDao) error) error {
	return dao.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		txDao := &UserAddressesDao{DB: tx}
		return fn(txDao)
	})
}
...
// buildUserAddressesQueryCondition 构建查询条件
// 参数:
//   - db: GORM数据库连接实例
//   - queryDto: 查询条件Dto对象
//
...
func (dao *UserAddressesDao) buildUserAddressesQueryCondition(db *gorm.DB, queryDto *query.UserAddressesDto) (*gorm.DB, error) {
	if queryDto == nil {
		return db, nil
	}
	...
	if queryDto.Province != "" {
		db = db.Where("province = ?", queryDto.Province)
	}
	if queryDto.City != "" {
		db = db.Where("city = ?", queryDto.City)
	}
	if queryDto.District != "" {
		db = db.Where("district = ?", queryDto.District)
	}
	if queryDto.DetailAddress != "" {
		db = db.Where("detail_address = ?", queryDto.DetailAddress)
	}
	if queryDto.IsDefault != nil {
		db = db.Where("is_default = ?", queryDto.IsDefault)
	}
	...
	if queryDto.ProvinceFuzzy != "" {
		db = db.Where("province LIKE ?", "%"+queryDto.ProvinceFuzzy+"%")
	}
	if queryDto.CityFuzzy != "" {
		db = db.Where("city LIKE ?", "%"+queryDto.CityFuzzy+"%")
	}
	if queryDto.DistrictFuzzy != "" {
		db = db.Where("district LIKE ?", "%"+queryDto.DistrictFuzzy+"%")
	}
	if queryDto.DetailAddressFuzzy != "" {
		db = db.Where("detail_address LIKE ?", "%"+queryDto.DetailAddressFuzzy+"%")
	}
...
//     2. List 不为 nil 且长度 > 0: 添加 IN 查询条件
//     3. List 不为 nil 但长度 = 0: 返回错误，因为空列表的 IN 查询没有意义，应提前发现此问题
func (dao *UserAddressesDao) SelectList(ctx context.Context, queryDto *query.UserAddressesDto) ([]*model.UserAddresses, error) {
	var resultList []*model.UserAddresses
	db := dao.WithContext(ctx).Model(&model.UserAddresses{})

	// 应用查询条件
	var err error
	db, err = dao.buildUserAddressesQueryCondition(db, queryDto)
	if err != nil {
		return nil, err
	}
...
//     2. List 不为 nil 且长度 > 0: 添加 IN 查询条件
//     3. List 不为 nil 但长度 = 0: 返回错误，因为空列表的 IN 查询没有意义，应提前发现此问题
func (dao *UserAddressesDao) SelectCount(ctx context.Context, queryDto *query.UserAddressesDto) (int64, error) {
	var count int64
	db := dao.WithContext(ctx).Model(&model.UserAddresses{})
...
func (dao *UserAddressesDao) SelectListWithAppendConditionFunc(ctx context.Context, queryDto *query.UserAddressesDto, appendConditionFunc func(ctx context.Context, db *gorm.DB) *gorm.DB) ([]*model.UserAddresses, error) {
	var resultList []*model.UserAddresses
	db := dao.WithContext(ctx).Model(&model.UserAddresses{})
...
func (dao *UserAddressesDao) SelectCountWithAppendConditionFunc(ctx context.Context, queryDto *query.UserAddressesDto, appendConditionFunc func(ctx context.Context, db *gorm.DB) *gorm.DB) (int64, error) {
	var count int64
	db := dao.WithContext(ctx).Model(&model.UserAddresses{})
...
func (dao *UserAddressesDao) Insert(ctx context.Context, poBean *model.UserAddresses) error {
	if poBean == nil {
...
func (dao *UserAddressesDao) InsertBatch(ctx context.Context, poBeanList []*model.UserAddresses) error {
	if len(poBeanList) == 0 {
...
func (dao *UserAddressesDao) InsertOrUpdateNullable(ctx context.Context, poBean *model.UserAddresses) error {
	if poBean == nil {
...
func (dao *UserAddressesDao) InsertOrUpdateBatchNullable(ctx context.Context, poBeanList []*model.UserAddresses) error {
	if len(poBeanList) == 0 {
...
// 说明:
//   - 该索引不是唯一索引，可能返回多条记录
func (dao *UserAddressesDao) SelectByUserId(ctx context.Context, userId uint64) ([]*model.UserAddresses, error) {
	var resultList []*model.UserAddresses
...
//   - [] *model.UserAddresses: 查询结果列表
//   - error: 错误信息
func (dao *UserAddressesDao) SelectByUserIdList(ctx context.Context, userIdList []uint64) ([]*model.UserAddresses, error) {
	if len(userIdList) == 0 {
		return []*model.UserAddresses{}, nil
	}
	var resultList []*model.UserAddresses
...
func (dao *UserAddressesDao) UpdateByUserId(ctx context.Context, poBean *model.UserAddresses, userId uint64) error {
	if poBean == nil {
		return fmt.Errorf("更新对象不能为空")
	}
	return dao.WithContext(ctx).Model(&model.UserAddresses{}).Where("user_id = ?", userId).Updates(poBean).Error
}
...
//   - 注意: 该索引不是唯一键，可能会更新多条记录
func (dao *UserAddressesDao) UpdateByUserIdWithMap(ctx context.Context, userId uint64, updatedMap map[string]interface{}) error {
	if len(updatedMap) == 0 {
		return fmt.Errorf("更新字段不能为空")
	}
	return dao.WithContext(ctx).Model(&model.UserAddresses{}).Where("user_id = ?", userId).Updates(updatedMap).Error
}
...
//   - 注意: 可能会更新多条记录
func (dao *UserAddressesDao) UpdateByUserIdWithCondition(ctx context.Context, poBean *model.UserAddresses, userId uint64, conditionMap map[string]interface{}) error {
	if poBean == nil {
		return fmt.Errorf("更新对象不能为空")
	}
	db := dao.WithContext(ctx).Model(&model.UserAddresses{}).Where("user_id = ?", userId)
...
//   - 使用 map 可以显式指定要更新的字段，包括零值字段
//   - 提供最灵活的更新控制方式
func (dao *UserAddressesDao) UpdateByUserIdWithMapAndCondition(ctx context.Context, userId uint64, updatedMap map[string]interface{}, conditionMap map[string]interface{}) error {
	if len(updatedMap) == 0 {
		return fmt.Errorf("更新字段不能为空")
	}
	db := dao.WithContext(ctx).Model(&model.UserAddresses{}).Where("user_id = ?", userId)
...
func (dao *UserAddressesDao) DeleteByUserId(ctx context.Context, userId uint64) error {
	return dao.WithContext(ctx).Where("user_id = ?", userId).Delete(&model.UserAddresses{}).Error
}
...
//
//	// 示例1: 查询单条记录
//	var result entity.UserAddresses
//	err := dao.ExecSql(ctx, &result, "SELECT * FROM user_addresses WHERE id = ?", 1)
//	if err != nil {
//	    // 处理错误（包括记录不存在的情况）
//	    return err
//	}
//
//	// 示例2: 查询多条记录
//	var resultList [] *entity.UserAddresses
//	err := dao.ExecSql(ctx, &resultList, "SELECT * FROM user_addresses WHERE skill_id = ?", "skill123")
//	if err != nil {
//	    return err
//	}
//
...
//	err := dao.ExecSql(ctx, &countList, "SELECT skill_id, COUNT(*) as count FROM user_addresses GROUP BY skill_id")
//
// 注意事项:
...
func (dao *UserAddressesDao) ExecSql(ctx context.Context, recvPtr any, sql string, args ...any) error {
...
func (dao *UserAddressesDao) getValidOrderByFields() map[string]bool {
...
func (dao *UserAddressesDao) isValidOrderBy(orderBy string) bool {
	if orderBy == "" {
		return false
	}
